...
## Ed merges and deploys through Claude, not by hand
- **Pattern:** After raising PR #8 I stopped and offered to watch it. Ed expected the PR to be merged and deployed in the same session, as in every previous session. Deploy is automatic: `.github/workflows/deploy.yml` runs on every push to `main` (build, rsync to VPS, `docker compose up --build`).
- **Rule:** When Ed asks for a PR on this repo, merge it (merge commit, matching PRs #4 to #8), then confirm the deploy run finished and probe the live page for the change. Only stop short of merging if the change is risky or he says review first.
