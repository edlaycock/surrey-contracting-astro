...
In `src/data/homepage.ts`: `QUOTE_TURNAROUND` (written quote turnaround after the site visit) and `EXTENSION_DURATION` (typical single-storey extension groundworks duration, first dig to slab). Two values, both used in the FAQ. Nothing else on the page is waiting on anyone.
...
1. **Build**: `npx astro build` completes clean, no warnings. `npm run build` currently exits 1 by design: the postbuild guard reports the two unfilled placeholders on the homepage, and nothing on any other page. Nothing else fails. Once `src/data/homepage.ts` is filled the same command goes green.
2. **Placeholders**: 2 distinct `{{` values on the built homepage, listed above. Enforced by `scripts/check-homepage.mjs` across every built HTML file, not just the homepage.
3. **Schema**: built homepage JSON-LD parses. Top-level nodes are `HomeAndConstructionBusiness` (with `knowsAbout` and `identifier` added; no `dateModified`, no `founder`) and `FAQPage` (11 questions). No Article, no aggregateRating, no Person (the first two guarded). validator.schema.org and the Rich Results Test against the LIVE URL, with screenshots, happen after deploy and are attached to the PR as a follow-up comment; the sandbox proxy refused a pre-deploy connection to the validator.
4. **DOM**: with JavaScript disabled (Playwright, `javaScriptEnabled: false`) the first `<p>` after the `<h1>` is "Surrey Contracting Limited is a groundworks, earthworks and demolition contractor..." and all 11 FAQ items are present in the server HTML as `<details>` with `<summary>` and `<p>`. Answer lengths 48, 51, 47, 65, 53, 46, 50, 41, 52, 45, 51 words. The only links inside answers are internal (service pages, the accreditations strip, the quote form, the phone number); no external links. The schema carries the same text with tags removed, and the guard compares the two.
5. **Word count**: 1,075 body-copy words (nav, footer, scripts, form controls excluded), reported by the guard on every build; the guard fails above 1,200. Simulated with realistic values in both placeholders: about 1,080.
