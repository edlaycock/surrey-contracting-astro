...
3. **Schema**: built homepage JSON-LD parses. Top-level nodes are `HomeAndConstructionBusiness` (with `knowsAbout` and `identifier` added; no `dateModified`, no `founder`) and `FAQPage` (10 questions). No Article, no aggregateRating, no Person (the first two guarded). validator.schema.org and the Rich Results Test against the LIVE URL, with screenshots, happen after deploy and are attached to the PR as a follow-up comment; the sandbox proxy refused a pre-deploy connection to the validator.
