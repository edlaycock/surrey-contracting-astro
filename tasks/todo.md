...
### Placeholders that must come from the business (build cannot deploy until every one is filled)

In `src/data/homepage.ts`: `QUOTE_TURNAROUND`, `EXTENSION_DURATION`, `REVIEWER_NAME` (the director signing off the content), `REVIEW_DATE` and `REVIEW_DATE_ISO`. Five values. No accreditation registration numbers are published anywhere on the site (Ed, 11 Sep), so none are needed.
...
- **Nothing under the accreditation logos.** The list of scheme names and register links that replaced the numbers is gone too. The accreditations section is the heading and the logo marquee only, as it was. The SSIP Portal and Constructionline links now sit inline in the "How do you check a contractor is legitimate?" FAQ answer, alongside the Companies House link, where the question calls for them. Easy to strip if not wanted.
...
1. **Build**: `npx astro build` completes clean, no warnings. `npm run build` currently exits 1 by design: the postbuild guard reports the unfilled placeholders on the homepage and one (`dateModified`) on every other page from the site-wide business node. Nothing else fails. Once `src/data/homepage.ts` is filled the same command goes green.
2. **Placeholders**: 5 distinct `{{` values on the built homepage, listed above. Enforced by `scripts/check-homepage.mjs` across every built HTML file, not just the homepage.
3. **Schema**: built homepage JSON-LD parses. Top-level nodes are `HomeAndConstructionBusiness` (with `dateModified`, `knowsAbout`, `identifier`) and `FAQPage` (11 questions). No Article, no aggregateRating, no Person (the first two guarded). validator.schema.org and the Rich Results Test against the LIVE URL, with screenshots, happen after deploy and are attached to the PR as a follow-up comment; the sandbox proxy refused a pre-deploy connection to the validator.
4. **DOM**: with JavaScript disabled (Playwright, `javaScriptEnabled: false`) the first `<p>` after the `<h1>` is "Surrey Contracting Limited is a groundworks, earthworks and demolition contractor..." and all 11 FAQ items are present in the server HTML as `<details>` with `<summary>` and `<p>`. Answer lengths 48, 51, 47, 65, 53, 46, 50, 41, 56, 45, 51 words. Links inside answers are plain `<a>` in the server HTML; the schema carries the same text with tags removed, and the guard compares the two.
5. **Word count**: 1,099 body-copy words (nav, footer, scripts, form controls excluded), reported by the guard on every build; the guard fails above 1,200. Simulated with realistic values in every placeholder: about 1,107.
