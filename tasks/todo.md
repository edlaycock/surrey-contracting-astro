...
In `src/data/homepage.ts`: `QUOTE_TURNAROUND`, `EXTENSION_DURATION`, `CHAS_NUMBER`, `CONSTRUCTIONLINE_NUMBER`, `SAFECONTRACTOR_NUMBER`, `SMAS_NUMBER`, `REVIEWER_NAME` (the director signing off the content), `REVIEW_DATE` and `REVIEW_DATE_ISO`. Nine values. Word budget after filling with realistic values: about 1,154 of 1,200.
...
2. **Placeholders**: 9 distinct `{{` values on the built homepage, listed above. Enforced by `scripts/check-homepage.mjs` across every built HTML file, not just the homepage.
...
5. **Word count**: 1,146 body-copy words (nav, footer, scripts, form controls excluded), reported by the guard on every build; the guard fails above 1,200. Simulated with realistic values in every placeholder: 1,154.
